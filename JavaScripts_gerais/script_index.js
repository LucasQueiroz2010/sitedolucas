...
if (!usuarioLogado) {
    ...
    if (usuarioLogado.banido) {
        alert("Calma aí! Sua conta foi banida temporariamente. Dê um tempo e respire.");
        localStorage.removeItem('usuarioAtual');
        window.location.href = "login.html";}

    const perfilWidget = document.getElementById('perfil-widget');
    if (perfilWidget) {
        perfilWidget.style.display = 'block';
        document.getElementById('nome-usuario-logado').innerText = usuarioLogado.nome;
        document.getElementById('foto-usuario-logado').src = usuarioLogado.foto || 'imagens/usuario_padrao.png';

        perfilWidget.addEventListener('click', () => {
            if (usuarioLogado.cargo === 'admin') {
                window.location.href = "admin.html";
            } else {
                window.location.href = "usuario.html";}});}

    var nomepessoal = usuarioLogado.nome;
    var genero = usuarioLogado.genero;
    var idadepessoal = null;

    if (usuarioLogado.dataNascimento) {
        idadepessoal = calcularIdade(usuarioLogado.dataNascimento);}

    // FUNÇÕES PARA A MENSAGEM CUSTOMIZADA.
    if (!nomepessoal) {
        document.getElementById("saudações").innerText = `Seja bem-vindo ao Site do Lucas, pessoa genérica sem nome!`;
        nomepessoal = null;}

    if (idadepessoal === null || isNaN(idadepessoal)) {
...
if (botaoEnviar) {
    botaoEnviar.addEventListener('click', function(e) {
        ...
        alert("Obrigado pelo seu feedback, " + usuarioLogado.nome + "!");
